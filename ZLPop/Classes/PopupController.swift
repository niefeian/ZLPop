 ...
 */
@objc public protocol PopupContentViewController {
    
    /** sizeForPopup(popupController: size: showingKeyboard:):
     return view's size
     */
    @objc func sizeForPopup(_ popupController: PopupController, size: CGSize, showingKeyboard: Bool) -> CGSize
}

@objc open class PopupController: UIViewController {
    
    @objc  public  enum PopupLayout : Int {
        case top = 0, center, bottom, centerSubNav
        ...
        func origin(_ view: UIView, size: CGSize = UIScreen.main.bounds.size) -> CGPoint {
            ...
            case .centerSubNav : return CGPoint(x: (size.width - view.frame.width) / 2, y: (size.height - view.frame.height) / 2 - 0)
            case .bottom: return CGPoint(x: (size.width - view.frame.width) / 2, y: size.height - view.frame.height)
            }
        }
    }
    
    @objc  public enum PopupAnimation : Int {
        case fadeIn = 0 , slideUp , none , left , right
    }
...
public extension PopupController {
    ...
    // MARK: Classes
    @objc class func create(_ parentViewController: UIViewController?) -> PopupController? {
       
//        if let vc = currentViewController()
//        {
//            if vc.classForCoder  != parentViewController?.classForCoder
//            {
//                return nil
//            }
//        }
    ...
    @objc func show(_ childViewController: UIViewController){
...
//        if let popView = childViewController as? PopupController{
...
//        }
    ...
    @objc func didCloseHandler(_ handler: @escaping (PopupController) -> Void) -> PopupController {
